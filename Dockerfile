...
#Installing fastagi server
WORKDIR /opt
RUN apt update && apt install -y wget xinetd &&  \
    wget https://netcologne.dl.sourceforge.net/project/phpagi/phpagi/2.20/phpagi-2.20.tgz && \
    tar zxvf phpagi-2.20.tgz \
    && mv phpagi-2.20 phpagi

RUN echo "fastagi    4573/tcp">> /etc/services

COPY fastagi /etc/xinetd.d/
